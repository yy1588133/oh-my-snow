...
2. **用 /oms:auto 执行**
   ...
   - 动作：用户显式选这项后，用 `slash command `/oms:auto` (not a skill — command prompt starts oms-start)` 配 spec 文件路径作为上下文。
...
4. **用 /oms:team 执行**
   ...
   - 动作：用 `slash command `/oms:team` (not a skill — multi-agent command prompt)` 配 spec 文件路径作为共享 plan。

5. **继续精炼**
   - 描述："继续访谈提高清晰度（当前：{score}%）"
   - 动作：回 Phase 2 interview loop。

**重要**：用户显式选执行后，**必须**桥接到真实入口：真实 skill（plan/ralph 等）用 `skill-execute`；`/oms:auto` 与 `/oms:team` 用 slash command（**禁止** `skill-execute oms/auto|team`）。**不要**直接实现。
...
- **直接实现**：用户选执行后自己写代码。skill 用 `skill-execute`，auto/team 用 slash command；禁止假 skill `oms/auto|team`。interview 是需求 skill，不是执行 skill。
...
| `slash command `/oms:auto` (not a skill — command prompt starts oms-start)` | Phase 5 桥接到 auto 自主执行 |
| `skill-execute { skill: "oms/ralph" }` | Phase 5 桥接到 ralph 持久循环 |
| `slash command `/oms:team` (not a skill — multi-agent command prompt)` | Phase 5 桥接到 team 并行协调 |
