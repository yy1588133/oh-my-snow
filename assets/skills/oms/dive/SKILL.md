...
- 已有 PRD 或 spec → 直接用 `/oms:auto` 或 `/oms:ralph` 带那个 plan 跑
...
1. **Ralplan → Autopilot（推荐）**
   ...
   - Action: `skill-execute { skill: "oms/plan", arguments: "--consensus --direct" }`，传 spec 文件路径（`spec_path` from state）当上下文。`--direct` 跳过 oms plan skill 的 interview 阶段（dive 的 interview 已经收集过需求），`--consensus` 触发 Planner/Architect/Critic 循环。共识完成、在 `.snow/oms-state/plans/` 产出 plan 后，`skill-execute { skill: "oms/auto" }` 带共识 plan 当 Phase 0+1 输出——autopilot 跳过 Expansion 和 Planning，直接从 Phase 2（执行）开始
   - 流水线: `dive spec → oms plan --consensus --direct → autopilot 执行`

2. **用 autopilot 执行（跳过 ralplan）**
   - Description: "全自动流水线——规划、并行实现、QA、验证。更快但没共识细化。"
   - Action: `skill-execute { skill: "oms/auto" }` 带 spec 文件路径当上下文。spec 替代 autopilot 的 Phase 0——autopilot 从 Phase 1（规划）开始
...
4. **用 team 执行**
   ...
   - Action: `skill-execute { skill: "oms/team" }` 带 spec 文件路径当共享 plan

5. **继续细化**
   - Description: "继续 interview 提升清晰度（当前: {score}%）。"
   - Action: 回 Phase 4 interview 循环

**IMPORTANT:** 选了执行后，**必须**用 `skill-execute` 桥接到对应 skill，带显式 `spec_path`。**不要**直接实现。dive skill 是需求流水线，不是执行 agent。
...
| `skill-execute { skill: "oms/auto" }` | 桥接 autopilot 执行（Phase 5） |
| `skill-execute { skill: "oms/ralph" }` | 桥接 ralph 执行（Phase 5） |
| `skill-execute { skill: "oms/team" }` | 桥接 team 执行（Phase 5） |
